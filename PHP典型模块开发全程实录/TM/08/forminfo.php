...
include_once("system/system.inc.php");								//包含类的实例化文件
$ddnumber=$_GET['fid'];												//获取订单号
$sql = "select * from tb_form where formid='".$_GET['fid']."'";		//定义SQL语句
$formarr = $admindb->ExecSQL($sql,$conn);							//执行SQL语句
$commname = explode(',',$formarr[0]['commo_name']);					//读取数据库中数据
$commnumber = explode(',',$formarr[0]['commo_num']);
$commagoprice = explode(',',$formarr[0]['agoprice']);				//读取数据库中数据
$commfold = explode(',',$formarr[0]['fold']);
$smarty->assign('formarr',$formarr[0]);								//将数据赋给模板变量
